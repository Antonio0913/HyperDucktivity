...
function NewTask({ addTask }) {
  ...
  return (
    <div>
      <button className="bg-background-gray text-beak-orange rounded-lg py-2 px-4" onClick={toggleForm}>{isOpen ? 'Cancel' : 'Add Task'}</button>
      {isOpen && (
       ...
       <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
         ...
         />
         <button className="self-end bg-background-gray text-beak-orange rounded-lg py-2 px-4" type="submit">
           Submit Task
         </button>
       </form>
     </div>
